...
url = "https://www.zomato.com/cincinnati/cincinnati-restaurants"
header = {
...
for each in searchList:
    data = {}
    data["restaurant"] = each.find(
        "a",{"data-result-type": "ResCard_Name"}).text.replace('\n',"").strip()
    data["locality"] = each.find("b").text.replace('\n',"").strip()
    data["ratings"] = each.find(
        "span",{"class": "rating-value"}).text.replace('\n',"").strip()
